  ...
  alloc {
    ...
    var def = SynthDef.new(\sin, {
      arg out, vol=0.0, hz=220, hz_lag=0.005,
        ...
        pan=0.0, pan_lag=0.005, mul=1, modPartial=1, carPartial=1, fm_index=0.0, sample_rate=48000, bit_depth=24;
...
//      car_decimate = Decimator.ar(car, sample_rate, bit_depth, 1.0, 0);
      Out.ar(out, Pan2.ar(SinOsc.ar(hz_) * vol_, pan_));
    });
